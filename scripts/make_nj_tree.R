...
map_col <- function(possible) {
  ...
  if (length(idx) > 0) return(names(metadata)[idx[1]])
  return(NA)
}
...
if (is.na(sample_col) || is.na(pop_col) || is.na(ploidy_col)) {
  stop(sprintf("Metadata must contain sample, population, ploidy. Found: sample=%s pop=%s ploidy=%s",
...
if (any(is.na(keep_idx))) {
  ...
  stop(sprintf("Sample ID mismatch: %d metadata sampleIDs not found in VCF: %s",
...
df_vc <- data.frame(component = rownames(res$varcomp), sigma2 = vc, P.value = pv,
...
df_vc <- data.frame(component = rownames(res_ploidy$varcomp), sigma2 = vc, P.value = pv,
                      percent = round(vc / sum(vc) * 100, 3), row.names = NULL)
write.table(df_vc, file = outf, sep = "\t", row.names = FALSE, quote = FALSE, append = TRUE)
# write also the varcoef
...
df_vc <- data.frame(component = rownames(res_all$varcomp), sigma2 = vc, P.value = pv,
                      percent = round(vc / sum(vc) * 100, 3), row.names = NULL)
write.table(df_vc, file = outf, sep = "\t", row.names = FALSE, quote = FALSE, append = TRUE)
# write also the varcoef
...
pdf(file.path(out_dir, "neighbournet_plot_pop_colours.pdf"), width = 10, height = 8)
  ...
  plot(nnet, edge.color = edge_cols, edge.width = ifelse(is_tip_edge, 2, 1),
...
pdf(file.path(out_dir, "splitsNetwork_plot_pop_colours.pdf"), width = 8, height = 8)
  op <- par(mar = c(1,1,1,1))
  # plot should accept edge.color / edge.width; use smaller tip labels and no margins
  plot(splitsnet, edge.color = edge_cols, edge.width = ifelse(is_tip_edge, 2, 1),
       tip.cex = 0.7, cex = 0.8, no.margin = TRUE)
  # add coloured tip labels (in case plot() didn't colour them)
  tiplabels(pch = 19, col = tip_cols, cex = 0.8)
  # legend for groups
...
if (length(missing_idx) > 0) {
  stop("Missing ploidy for samples (ploidy must be provided for all tips): ",
...
if (length(bad_idx) > 0) {
  stop("Unrecognized ploidy values for samples: ",
...
pdf(file.path(out_dir, "neighbournet_plot_ploidy_colours.pdf"), width = 10, height = 8)
  ...
  legend("topright", legend = names(ploidy_colors), col = ploidy_colors, pch = 19,
