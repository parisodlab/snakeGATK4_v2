...
def get_genes_from_annotation(annotation_file):
    """Parse a GFF/GFF3 annotation and return a dict: {contig: [(gene_start, gene_end), ...]}.

    This version groups genes by the contig (first column) and does not rely on
    special 'region' lines. It skips comment lines and sorts gene ranges by start.
    """
    contigs = {}
    opener = gzip.open if annotation_file.endswith(".gz") else open
    with opener(annotation_file, 'rt') as annotation:
        for line in annotation:
            ...
            if is_gene_line(line):
                ...
                contigs.setdefault(contig, []).append((gene_start, gene_end))
    # sort ranges for each contig by start position
    for contig in contigs:
        contigs[contig].sort(key=lambda r: r[0])
    return contigs
...
if __name__ == "__main__":
    main()
# End of file - single main() and guard above handle execution
